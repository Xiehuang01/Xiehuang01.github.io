...
// 修改图片展开功能
function expandCard(scoreCard) {
    const title = scoreCard.querySelector('h3').textContent;
    const imgSrc = scoreCard.querySelector('img').src;
    const note = scores.find(score => score.title === title).note;
    ...
    scoreCard.innerHTML = `
        <div class="expanded-header">
            <button class="close-btn" onclick="closeExpandedCard()">
...
function closeExpandedCard() {
    ...
    if (expandedCard) {
        expandedCard.style.transform = 'translateX(100%)';
        setTimeout(() => {
            displayScores(scores); // 重新渲染所有卡片
            overlay.style.display = 'none';
